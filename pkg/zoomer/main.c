...
static struct SDL_Window *initialize_window(const char *name, int w, int h) {
	...
	if (w == FULLSCREEN_W && h == FULLSCREEN_H) flag |= SDL_WINDOW_FULLSCREEN_DESKTOP;

	window = SDL_CreateWindow(name, SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, w, h, flag);
	if (NULL == window) SDL_DIE(window);
...
static struct SDL_Renderer *initialize_renderer(struct SDL_Window *window) {
	...
	if (NULL == renderer) SDL_DIE(renderer);
...
static void install_zoomer(struct Zoomer *zoomer, const char *name, int w, int h) {
	...
	if (SDL_Init(SDL_INIT_VIDEO) < 0) SDL_DIE(zoomer);
	...
	if (!SDL_GetWindowWMInfo(zoomer->window, &info)) SDL_DIE(info);
	...
	// Take screenshot
	image = XGetImage(
	    display, root, 0, 0, zoomer->root_attributes.width, zoomer->root_attributes.height,
	    AllPlanes, ZPixmap);
	if (NULL == image) SDL_DIE(image);

	// Image width and height will be the same as in the attributes we store
	i = 0;
	zoomer->image = malloc((sizeof *zoomer->image) * (image->width * image->height));
...
static float nearest_multiple(float x, float m) {
	return m * floorf(x / m);
}

static void snap_to_grid(
    const SDL_Rect *stat, const SDL_Rect *dyn, int mx, int my, float *x, float *y, float *w,
    float *h) {
...
int main(int argc, char *argv[]) {
	...
	fwrite(
	    zoomer.image, zoomer.root_attributes.width * zoomer.root_attributes.height,
	...
	if (NULL == buffer) SDL_DIE(buffer);

	// Write image to buffer
	pitch = zoomer.root_attributes.width * (sizeof *zoomer.image);
	if (SDL_UpdateTexture(buffer, NULL, (const void *)zoomer.image, pitch) != 0) SDL_DIE(buffer);
	...
	if ((disps = SDL_GetNumVideoDisplays()) < 0) SDL_DIE(disps);
	...
	for (;;) {
		...
		if (SDL_PollEvent(&e)) {
			...
			if (e.type == SDL_KEYUP) {
				...
				if (e.type == SDL_KEYDOWN && e.key.keysym.sym > SDLK_0 &&
					...
					if (dispidx <= disps) {
						if (SDL_GetDisplayBounds(dispidx - 1, &stat) != 0) SDL_DIE(stat);
		...
		if (grid) {
			...
			for (i = 1; i < (stat.w / (int)dw); i++) {
				SDL_RenderDrawLineF(
				    zoomer.renderer, ((float)i * dw), 0.f, (i * (float)dw), (float)stat.h);
			}

			for (i = 1; i < (stat.h / (int)dh); i++) {
				SDL_RenderDrawLineF(
				    zoomer.renderer, 0.f, ((float)i * dh), (float)stat.w, ((float)i * dh));
			}
